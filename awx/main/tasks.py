...
from awx.main.utils import (get_ssh_version, decrypt_field, update_scm_url,
                            get_licenser,
...
class BaseTask(object):
    ...
    cleanup_paths = []
    proot_show_paths = []
    ansible_runner_output_filter = 'event'
    ...
    def run(self, pk, **kwargs):
        ...
        try:
            ...
            if instance.is_isolated() is True:
                ...
                def finished_callback(instance):
                    pass

                def status_handler(instance, status):
                    if status == 'canceled':
                        # TODO: if we update instance here, is it updated outside?
                        self.update_model(instance.pk, status=status)
                ...
                params = {
                    ...
                    'status_handler': functools.partial(status_handler, instance),
                    'cancel_callback': functools.partial(cancel_callback, instance),
                    'output_filter': self.ansible_runner_output_filter,
                    'settings': {
                ...
                for v in ['passwords', 'playbook', 'inventory']:
                    if not params[v]:
                        del params[v]
                '''
                Ansible Runner does not respect the PYTHONPATH passed via
                environment variables. Instead, it gets PYTHONPATH from the
                current process environment. Thus the below hack.
                '''
                old_pythonpath = os.getenv('PYTHONPATH', '')
                new_pythonpath = env.get('PYTHONPATH')
                if new_pythonpath:
...
class RunJob(BaseTask):
    ...
    def build_passwords(self, job, runtime_passwords):
        ...
        if cred:
            for field in ('ssh_key_unlock', 'ssh_password', 'become_password', 'vault_password'):
                value = runtime_passwords.get(field, decrypt_field(cred, 'password' if field == 'ssh_password' else field))
                if value not in ('', 'ASK'):
        ...
        for cred in job.vault_credentials:
            ...
            value = runtime_passwords.get(field, decrypt_field(cred, 'vault_password'))
            if value not in ('', 'ASK'):
                passwords[field] = value
        ...
        if 'ssh_key_unlock' not in passwords:
            for cred in job.network_credentials:
                if cred.inputs.get('ssh_key_unlock'):
                    passwords['ssh_key_unlock'] = runtime_passwords.get('ssh_key_unlock', decrypt_field(cred, 'ssh_key_unlock'))
                    break
    ...
    def build_env(self, job, private_data_dir, isolated=False, private_data_files=None):
        ...
        if job.use_fact_cache:
            ...
            env['ANSIBLE_CACHE_PLUGIN_CONNECTION'] = os.path.join(private_data_dir, 'facts')
        if job.project:
    ...
    def final_run_hook(self, job, status, private_data_dir, fact_modification_times):
        ...
        custom_stats_path = os.path.join(private_data_dir, 'artifacts', 'custom')
...
class RunInventoryUpdate(BaseTask):
    ...
    ansible_runner_output_filter = 'newline'
